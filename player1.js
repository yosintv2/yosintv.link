...
function initializePlayer() {
    try {
        ...
        const safeSrc = urlParams.get('src');  // URL-safe Base64 string
        const safeHls = urlParams.get('hls');  // URL-safe Base64 string
        ...
        if (safeSrc) {
            const srcUrl = decodeUrlSafeBase64(safeSrc);
            iframe.src = srcUrl;
            jwPlayerDiv.style.display = 'none';
        } else if (safeHls && typeof jwplayer === 'function') {
            const hlsUrl = decodeUrlSafeBase64(safeHls);
